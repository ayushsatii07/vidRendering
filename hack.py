...
def process_segment(segment_path: str, filter_type: str, temp_dir: str) -> str:
    ...
    # ✅ STRATEGY 3: Force single-threaded encoding to avoid competition
    # Use medium preset + disable internal threading
    cmd = [
        "ffmpeg",
        "-i", segment_path,
        "-vf", vf_filter,
        "-map", "0:v",
        "-c:v", "libx264",
        "-preset", "medium",          # ✅ Medium preset (not too slow)
        "-threads", "1",              # ✅ CRITICAL: 1 thread per process
        "-crf", "18",
        "-pix_fmt", "yuv420p",
        "-y",
        str(output_file)
